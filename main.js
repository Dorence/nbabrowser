...
function Team(abbr, id, idTencent, idHupu, code, enName, chName, pyName, enCity, chCity, enSta, region, division) {
    this.abbr = abbr;
    this.id = id; this.idT = idTencent; this.idH = idHupu;
    this.code = code;
    this.en = enName; this.cn = chName; this.py = pyName.split(" ");
    this.ec = enCity; this.cc = chCity; this.eSt = enSta;
    this.reg = region; this.div = division;
}
...
function aj(d)
{
    ...
    $.ajax({
        ...
        type: "GET",
        // dataType: "jsonp",
        // jsonpCallback: "ajaxExec",
        dataType: "text",
        data: "opt=tx&startTime=" + stime + "&endTime=" + etime,
		cache: false,
        success: function (t)
        {
            ajson = JSON.parse(t);
...
function init()
{
    ...
    $("#datetimepicker").datetimepicker().on("changeDate", (m) => {
        console.log(m);
        aj(m.date);
    });
